...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Auth gates for: Topics
        Gate::define('topic_access', function ($user) {
            return in_array($user->role_id, [1, 2, 3]);
        });
        Gate::define('topic_create', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
        Gate::define('topic_edit', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
        Gate::define('topic_view', function ($user) {
            return in_array($user->role_id, [1, 2, 3]);
        });
        Gate::define('topic_delete', function ($user) {
            return in_array($user->role_id, [1]);
        });
